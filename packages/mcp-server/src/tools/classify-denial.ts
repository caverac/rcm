...
import { ClassifyDenialInput, DenialClassification, DenialCodeLibrary } from '../types.js';

export async function classifyDenial(input: ClassifyDenialInput): Promise<DenialClassification> {
...
function classifyByPattern(code: string, text?: string): DenialClassification {
  ...
  let prevention_tips = 'Ensure all claim requirements are met before submission';
  ...
  if (prefix === 'CO') {
    ...
    if (number === '197') {
      ...
      description = 'Procedure code inconsistent with modifier or modifier missing';
      ...
      common_resolution = 'Review bundling rules or appeal if services were distinct';
    } else if (number === '29') {
      ...
      common_resolution = 'Verify coverage dates and appeal if service was covered';
    } else if (number === '96') {
      category = 'NON_COVERED';

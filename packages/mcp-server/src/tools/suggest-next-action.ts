...
import { SuggestNextActionInput, NextActionSuggestion, OrgPolicy, DenialCodeLibrary } from '../types.js';
import { classifyDenial } from './classify-denial.js';

export async function suggestNextAction(input: SuggestNextActionInput): Promise<NextActionSuggestion> {
  ...
  let recommended_action: NextActionSuggestion['recommended_action'] = 'investigate';
  let reason = '';
  ...
  if (!classification.is_appealable) {
    if (classification.category === 'PATIENT_RESPONSIBILITY') {
      ...
      reason = 'Patient responsibility amounts should be transferred to patient balance';
      priority = 'medium';
      days_to_action = 5;
    ...
    for (const policy of policies) {
      if (policy.policy_type === 'write_off_threshold') {
        ...
        if (policy.min_amount && denial_amount >= policy.min_amount) {
          ...
          priority = classification.category === 'AUTHORIZATION' ? 'high' : 'medium';
          days_to_action = policy.days_to_action || 30;
          auto_process = policy.auto_appeal;
          policy_applied = policy.policy_name;

          // Estimate recovery chance based on denial type
          estimated_recovery_chance = estimateRecoveryChance(classification.category);
    ...
    if (recommended_action === 'investigate') {
      if (classification.category === 'CODING_ERROR' || classification.category === 'MISSING_INFO') {
        recommended_action = 'rebill';

...
export async function batchClassifyDenials(input: BatchClassifyDenialsInput): Promise<DenialAnalytics> {
  ...
  const groupMap = new Map<string, { count: number; total_amount: number; codes: Set<string>; payer_ids: Set<string> }>();
  ...
  const highVolumeLowValue = groups.filter(g => g.count >= 5 && g.avg_amount < 100);
  ...
  const lowVolumeHighValue = groups.filter(g => g.count < 5 && g.avg_amount >= 500);
  ...
  // Preventable denials
  const preventableCategories = ['CODING_ERROR', 'MISSING_INFO', 'AUTHORIZATION', 'TIMELY_FILING'];
  const preventable = groups.filter(g => preventableCategories.includes(g.group_key));
  if (preventable.length > 0) {
    const preventableAmount = preventable.reduce((sum, g) => sum + g.total_amount, 0);
    const preventablePercent = Math.round((preventableAmount / total_amount) * 10000) / 100;
    insights.push(

...
sequenceDiagram
    ...
    Dev->>CDK: yarn deploy
    CDK->>CF: Create stack
    CF->>RDS: Create PostgreSQL instance
    RDS-->>CF: Instance ready + auto-generated password
    CF->>SM: Create /rcm/db-credentials
    CF->>SM: Populate with host, port, password, connection_string
    SM-->>Dev: Secret available
    Dev->>SM: Get DATABASE_URL
    Dev->>RDS: Connect & run migrations
```
